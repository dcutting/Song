...
class EvalTests: XCTestCase {
    ...
    func test_description() {
        assertNoThrow {
            ...
            let closure = try Expression.function(function).evaluate(context: .empty)
    ...
    func test_evaluate_closureWithoutParameters() {
        assertNoThrow {
            ...
            let closure = try function.evaluate(context: .empty)
            let call = Expression.eval(closure, [])
            let result = try call.evaluate(context: .empty)
            XCTAssertEqual(Expression.int(5), result)
        }
    }
    ...
    func test_evaluate_lambda() {
        ...
        assertNoThrow {
            let lambda = try Expression.function(function).evaluate(context: .empty)
