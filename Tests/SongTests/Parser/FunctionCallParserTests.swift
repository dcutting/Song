...
class FunctionCallParserTests: XCTestCase {
    ...
    func test() {
        ...
        "foo(|x|x.foo(bar()())(),1)".makes(.call("foo", [.eval(.lambda([.name("x")], .call("foo", [.name("x"), .eval(.call("bar", []), [])])), []), .int(1)]))
        ...
        "foo(1).foo(2)(3).bar".ok() // # .call(bar, .eval(.call(foo, .call(foo, [.int(1), [.int(2), [.int(3))
        "(foo).bar".ok() //        # .call(bar, .var(foo))
        "(foo(2)).bar".ok() //    # .call(bar, .call(foo, [.int(2))
        "1.(foo.bar).bar".ok() //  # .call(bar, .eval(.call(bar, .var(foo)), [.int(1))
        "(foo.bar).bar".ok() //    # .call(bar, .call(bar, .var(foo)))
        "foo(bar())".ok() //       # .call(foo, .call(bar))
        "foo(4.bar)".ok() //       # .call(foo, .call(bar, [.int(4))
        "foo(4.bar())".ok() //     # .call(foo, .call(bar, [.int(4))
        "foo().bar".ok() //        # .call(bar, .call(foo))
        "foo()().bar".ok() //      # .call(bar, .eval(.call(foo)))
        "foo.bar()()".ok() //      # .eval(.call(bar, .var(foo)))
        "foo(4.foo(bar(2))).(foo()().bar).bar(3)".ok() //  # .call(bar, ... , [.int(3)
        "(|x|x)(1)".ok() //        # .eval(|x|x, 1)
        "(|x|x).foo(1)".ok() //    # .call(foo, |x|x, 1)
        "(x)(1)(2,3).bar".ok() //  # .call(bar, .eval(.eval(x, [.int(1), [.int(2, [.int(3))
        "(|x|x)(1)(2,3).bar".ok() //    # .call(bar, .eval(.eval(|x|x, [.int(1), [.int(2, [.int(3))
        "(|x|x)().bar".ok() //     # .call(bar, .eval(|x|x))
        "1.(|x|x)".ok() //         # .eval(|x|x, 1)
        "1.(|x|x)()".ok() //       # .eval(.eval(|x|x, [.int(1))
        "foo().(|x|x)()".ok() //   # .eval(.eval(|x|x, .call(foo)))
        ...
        "x.foo(2)(3).bar".ok() //  # .call(bar, .call(foo, [.int(1)], [.int(2)]))
        "1.foo".ok() //            # .call(foo, [.int(1)])
        "lessthan(5)(4)".ok() //   # .eval(.call(lessThan, [.int(5)]), [.int(4)])
        "(foo)()".ok() //
        "(1+2).foo".ok() //
